function [SNR, estim_DOAvec, I] = calc_DOA_SRP(target_true_loc, i, R0, prepro)
    ...
    figure_mode = 0; % if enabled, every intensity map will be displayed on a figure.
    ...
    mic_config = 'anti_prism_mic_array'; % all microphone configuration files are found in the mic_array folder.
    ...
    if prepro
        I = SNR_to_r(true_loc, micPos, M, win, N_STFT, R_STFT, noise_mul);
    end
    ...
    for true_loc_idx = 1:loc_num
        ...
        if set_loc_mode
            disp(['PROCESSING SOURCE LOCATION ' num2str(i)])
        else
    ...
    %%
        ...
        Tictoc(true_loc_idx) = Tictoc(true_loc_idx) + toc;
    ...
    if set_loc_mode
        disp(['time per target: ' num2str(mean(Tictoc))])
    else
